...
```{r tfKEGG10_hp, fig.height= 6, fig.width= 14, fig.cap="TF-KEGG regulatory pairs that were significant in all conditions*"}
temp <- TF_KEGG %>%
  lapply(mutate, pair = paste(TF, gs_name, sep = "-")) %>%
...
Those consistently significant regulatory paris could also be visualised through a network plot. The red color of the edges reflected the activating effects those TFs potentially had on the KEGG pathways. 

```{r fig.height= 7 , fig.width= 8,  fig.cap="*Network plot of TF-KEGG regulatory pairs that were significant in all conditions*"}
temp <- TF_KEGG %>%
  lapply(mutate, pair = paste(TF, gs_name, sep = "-")) %>%
  lapply(dplyr::filter, pair %in% tfKEGG10) %>%
  bind_rows() %>%
...
```{r fig.height= 12 , fig.width= 14,  fig.cap="*Network plot of TF-KEGG regulatory pairs that were unique to SH-5Y5Y cell line*"}
temp <- TF_KEGG %>%
  lapply(mutate, pair = paste(TF, gs_name, sep = "-")) %>%
    lapply(dplyr::filter, pair %in% pairToExtract) %>%
  bind_rows() %>%
  ...
  group_by(TF, gs_name) %>%
  summarise(Mean_mad = mean(Z_mad)) %>%
  mutate(color = ifelse(Mean_mad > 0 ,"pink", "lightblue"),
         type = TF, 
         gs_name = vapply(gs_name, function(x){ifelse(length(strsplit(x, " ")[[1]]) > 2,
                                                                                     str_replace_nth(x, " ", "\n", 2),
                                                                                     x)}, character(1))) %>%
  dplyr::rename(from = TF, 
                to = gs_name) 
